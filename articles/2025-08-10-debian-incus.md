...
イミュータブルな機能が不要なのに、頑張ってDockerコンテナを使って苦労している人向けです。
...
私の場合は、btrfsを使っておりpoolの部分については内容を確認して納得して初期化しました。
...
`incus shell vibe`で、rootになったら、すぐに気がつくとおもいますが、hostnameがtrixieになっています。これだと困るのでhostnameコマンドでホスト名を変えておきましょう。具体的には`hostname vibe`です。
