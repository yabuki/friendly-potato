...
最近はありがたいことにUTF-8のシステムが増えて、日本語など言語が、コンピュータ上で表示しやすくなりました。

コンピュータ上で複数の言語を扱うには複数の方法があります。その中でのメジャーな実装が、gettextです。
...
gettext はプログラムの言語を切り替えるために使われてきましたが、現在では、[man po4a (1): PO ファイルと翻訳済みドキュメントを一括更新](http://ja.manpages.org/po4a) を使って文書の翻訳に使う例があります。
...
例題はシェルスクリプトですが、gettext における作業手順について他のプログラム言語でも応用ができるように書きます。
